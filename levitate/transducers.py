    ...
    def pressure_derivs(self, source_positions, source_normals, receiver_positions, orders=3):
        ...
        for ii in range(len(self.selected_modes)):
            ...
            if orders > 2:
                ...
                this_mode_derivatives[12] = constant * (np.e**(1j*n*receiver_positions_cyl[1])*m**3*np.pi**3*jv(n,(k_ns*receiver_positions_cyl[0])/self.radius)*np.sin((m*np.pi*receiver_positions_cyl[2])/self.height))/self.height**3
